...
void InfluxWorker::log_fields_map(const std::unordered_map<std::string, std::string>& fields)
{
  if (fields.empty()) {
...
std::unordered_map<std::string, std::string> InfluxWorker::parse_flux_fields(const std::string& resp)
{
...
InfluxWorker::InfluxWorker(const rtrs::DatabaseConfig config_) :
